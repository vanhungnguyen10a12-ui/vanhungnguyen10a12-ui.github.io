...
export default function ProductDetailPage({ params }) {
    ...
    const handleAddToCart = () => {
        if (!product) return;
        if (user) { alert(`Đã thêm ${product.name} vào giỏ hàng!`); } 
        else { router.push(`/login?redirect=/products/${product.id}`); }
    };
            ...
            <main className="flex-1 bg-brand-dark-light">
                <div className="container mx-auto px-4 py-12 sm:px-6 sm:py-20">
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                        <Link href="/#products" className="inline-flex items-center gap-2 text-sm text-neutral-400 hover:text-hineon-blue-light"><ChevronLeft size={16} />Quay lại danh sách sản phẩm</Link>
                        <div className="mt-8 grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-16">
                            ...
                            <motion.div className="flex flex-col" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.7, delay: 0.4 }}>
                                ...
                                <div className="mt-8 flex-1 border-t border-brand-dark-lighter pt-8"><p className="text-4xl font-bold text-white">{price.toLocaleString('vi-VN')} ₫</p></div>
                                <button onClick={handleAddToCart} className="mt-8 w-full rounded-lg bg-hineon-blue py-4 text-base font-semibold text-white shadow-2xl shadow-hineon-blue/30 transition-all hover:scale-105 hover:bg-hineon-blue-light hover:text-brand-dark">Thêm vào giỏ hàng</button>
                            </motion.div>
                        </div>
                    </motion.div>
                </div>
