...
import { LogOut, Shield } from 'lucide-react'; // Import icon Shield
                    ...
                    : user ? (
                        <div className="group relative">
                            ...
                            <div className="absolute right-0 top-full mt-2 w-48 origin-top-right rounded-md bg-brand-dark-light shadow-lg opacity-0 transition-all duration-200 group-hover:opacity-100">
                                <div className="py-1">
                                    {isAdmin && (
                                        <Link href="/admin" className="flex w-full items-center gap-2 px-4 py-2 text-left text-sm text-neutral-300 hover:bg-brand-dark-lighter">
                                            <Shield size={16} />
                        ...
                        <Link href="/login" className="hidden sm:inline-block rounded-lg bg-hineon-blue px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-hineon-blue/20 transition-all hover:scale-105 hover:bg-hineon-blue-light hover:text-brand-dark">Đăng nhập</Link>
                    )}
                </div>
            </div>
        </header>
