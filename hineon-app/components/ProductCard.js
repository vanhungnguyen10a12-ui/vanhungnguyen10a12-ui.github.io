...
function ProductCard({ product }) {
  ...
  // Kiểm tra an toàn: Nếu không có product, không render gì cả
  ...
  const {
    ...
    imageUrl = "https://placehold.co/800x600/161B22/7DF9FF?text=HINEON",
    price = 0,
    tag
  } = product;
  ...
  return (
    <motion.div variants={cardVariants}>
      <Link href={`/products/${id}`} className="group relative block overflow-hidden rounded-xl bg-brand-dark-light transition-all duration-300">
        ...
        <div className="relative flex flex-col rounded-lg bg-brand-dark-light p-1">
          <div className="relative h-60 w-full overflow-hidden rounded-lg">
            <Image src={imageUrl} alt={name} fill sizes="33vw" className="object-cover transition-transform duration-500 group-hover:scale-105" />
          </div>
          {tag && ( <div className="absolute top-4 left-4 z-10 rounded-full bg-brand-dark/50 px-3 py-1 text-xs font-bold text-hineon-blue-light backdrop-blur-sm">{tag}</div> )}
          <div className="flex flex-1 flex-col p-4">
            <h3 className="text-lg font-bold text-white">{name}</h3>
            <p className="mt-2 flex-1 text-sm text-neutral-400 h-10">{description.substring(0, 80)}</p>
            <div className="mt-4 flex items-center justify-between">
              <p className="text-xl font-extrabold text-white">{price.toLocaleString('vi-VN')} ₫</p>
